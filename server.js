...
function publicPlayer(p,i){return {id:p.id,name:p.name,bot:p.bot,botType:p.botType,connected:p.connected||p.bot,host:p.host,chips:p.chips,folded:p.folded,allIn:p.allIn,bet:p.bet,lastAction:p.lastAction,action:p.action,seat:i}}
...
function rouletteSpin(room){if(room.roulette.phase!=='BETTING')return;room.roulette.phase='SPINNING';broadcast(room);setTimeout(()=>{const n=crypto.randomInt(38);const result=n===37?'00':n===0?'0':String(n);room.roulette.result=result;room.roulette.phase='RESULT';for(const b of room.roulette.bets){const p=room.players.find(x=>x.id===b.playerId);if(!p)continue;let win=false,pay=0;const num=result==='00'?37:Number(result);if(b.type==='number')win=String(num)===String(b.value);if(b.type==='red')win=result!=='0'&&result!=='00'&&RED.has(num);if(b.type==='black')win=result!=='0'&&result!=='00'&&!RED.has(num);if(b.type==='odd')win=num>0&&num<37&&num%2===1;if(b.type==='even')win=num>0&&num<37&&num%2===0;if(b.type==='low')win=num>=1&&num<=18;if(b.type==='high')win=num>=19&&num<=36;if(win){const mult=b.type==='number'?35:1;pay=b.amount*(mult+1);p.chips+=pay}}say(room,`Roulette result: ${result}.`);broadcast(room);setTimeout(()=>{room.status='LOBBY';room.roulette={phase:'BETTING',bets:[],result:null};broadcast(room)},3500)},3500)}
...
io.on('connection',socket=>{
  socket.on('createRoom',({name}={},ack)=>{const p=player({name,socketId:socket.id});const room=newRoom(p);rooms.set(room.code,room);socket.join(room.code);socket.data.room=room.code;socket.data.player=p.id;say(room,`${p.name} created the room.`);broadcast(room);ack?.({ok:true,code:room.code,reconnectToken:p.reconnectToken})});
  socket.on('joinRoom',({code,name}={},ack)=>{const room=rooms.get(String(code||'').toUpperCase());if(!room)return ack?.({ok:false,error:'Room does not exist.'});if(room.players.length>=MAX_PLAYERS)return ack?.({ok:false,error:'Room is full.'});if(room.status==='IN_GAME')return ack?.({ok:false,error:'That room is already in a game.'});const p=player({name,socketId:socket.id});room.players.push(p);socket.join(room.code);socket.data.room=room.code;socket.data.player=p.id;say(room,`${p.name} joined the room.`);broadcast(room);ack?.({ok:true,code:room.code,reconnectToken:p.reconnectToken})});
