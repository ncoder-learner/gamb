...
function startPokerHand(room){
  clearTimer(room); const eligible=room.players.filter(p=>p.chips>0 && (p.connected || p.bot)); if(eligible.length<2){room.status='WAITING';room.poker.phase='LOBBY';broadcast(room);return;}
...
function nextHand(room){ if(!rooms.has(room.code))return; const count=room.players.filter(p=>p.chips>0&&(p.connected||p.bot)).length;if(count>=2){room.poker.dealerIndex=(room.poker.dealerIndex+1)%room.players.length;startPokerHand(room);}else{room.status='WAITING';room.poker.phase='LOBBY';broadcast(room);} }
...
io.on('connection',socket=>{
  ...
  socket.on('startGame',({entryBet}={},ack)=>{try{const room=rooms.get(socket.data.room),p=room?.players.find(x=>x.id===socket.data.player);if(!room||!p)return ack?.({ok:false,error:'You are not in a room.'});if(room.hostId!==p.id)return ack?.({ok:false,error:'Only the host can start.'});if(room.status==='IN_GAME')return ack?.({ok:false,error:'A game is already running.'});if(room.game==='poker'){const eligible=room.players.filter(x=>x.chips>0&&(x.connected||x.bot));if(eligible.length<2)return ack?.({ok:false,error:'Add at least one bot or another player before starting.'});startPokerHand(room);}else if(room.game==='blackjack')bjStart(room);else if(room.game==='roulette')rouletteStart(room);else chessStart(room,entryBet);broadcast(room);emitPrivate(room);ack?.({ok:true,game:room.game,status:room.status,phase:room.poker.phase});}catch(e){console.error('startGame failed:',e);ack?.({ok:false,error:'Server could not start the game.'});}});
