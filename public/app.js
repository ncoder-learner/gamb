...
function ackCall(event,payload){return new Promise(resolve=>socket.emit(event,payload,r=>resolve(r||{})));}
...
function card(c){if(c==='??')return '<div class="card back">PH</div>';const red=/[♥♦]/.test(c);return `<div class="card ${red?'red':''}">${esc(c)}</div>`;}
